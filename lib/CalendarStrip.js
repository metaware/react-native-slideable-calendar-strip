...
class DateItem extends PureComponent {
  render() {
    ...
    const highlightBgColor = '#0A6CF1';
    const normalBgColor = '#F8FAFB';
          ...
          <View style={[
            ...
            {marked &&
              <View style={[
                ...
                {backgroundColor: highlight ? 'white' : '#0A6CF1'}
              ]} />
            }
          </View>
        </TouchableOpacity>
...
class CalendarStrip extends Component {
  constructor(props) {
    ...
    if (this.props.onSwipe) {
      this.props.onSwipe(this.state.datas[0], this.state.datas[this.state.datas.length - 1])
    }
  }
  ...
  loadNextTwoWeek(originalDates) {
    ...
    if (this.props.onSwipe) {
      this.props.onSwipe(eachDays[0], eachDays[eachDays.length-1])
    }
  }
